...
window.onload = () => {
  fetch(`${BASE_URL}/pokemon?offset=0&limit=12`)
    .then((data) => data.json())
    .then((data) => {
      pokemonsListNames = data.results;
      renderPokemons(pokemonsListNames);
    });
};

const createPokeItems = (pokemonsListNames) =>
  pokemonsListNames.map((pokemon) => `<li>${pokemon.name}</li>`);

input.addEventListener(`keyup`, (e) => {
  const newPpokemons = pokemonsListNames.filter((pokemon) =>
    pokemon.name.includes(input.value)
  );
